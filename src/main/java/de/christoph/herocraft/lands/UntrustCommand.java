...
public class UntrustCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if(land.isModeratorUUID(player.getUniqueId().toString()) || land.isOwnerUUID(player.getUniqueId().toString())) {
            if(strings.length != 1) {
                player.sendMessage(Constant.PREFIX + "§7Bitte benutze §c/untrust <Spieler>§7.");
                return false;
            }
            ...
            land.unTrustPlayer(target);
            player.sendMessage(Constant.PREFIX + "§7Spieler §cnicht mehr getrusted§7.");
        } else {
