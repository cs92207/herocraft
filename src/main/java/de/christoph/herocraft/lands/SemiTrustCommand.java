...
public class SemiTrustCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if(land.isModeratorUUID(player.getUniqueId().toString()) || land.isOwnerUUID(player.getUniqueId().toString())) {
            if(strings.length != 1) {
                player.sendMessage(Constant.PREFIX + "§7Bitte benutze §c/semitrust <Spieler>§7.");
                return false;
            }
            ...
            land.semiTrustPlayer(target);
            player.sendMessage(Constant.PREFIX + "§7Spieler §agesemitrusted§7.");
        } else {
